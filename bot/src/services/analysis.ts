...
export const analyzeUserMessages = async (params: {
  chatId: number;
  userId: number;
  displayName: string;
}): Promise<{ analysis: string; messageCount: number; lookbackDays: number }> => {
  ...
  if (messages.length === 0) {
    return { analysis: 'Недостаточно сообщений для анализа.', messageCount: 0, lookbackDays: config.analyzeLookbackDays };
  }
  ...
  const analysis = await generateWithRetry(prompt, 2);
  const finalAnalysis = analysis
    ? analysis
    : buildFallbackAnalysis({ averageLength, peakHours, messages: messageTexts });

  return { analysis: finalAnalysis, messageCount: messages.length, lookbackDays: config.analyzeLookbackDays };
};
